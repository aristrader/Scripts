...
input_file = '/Users/swapnilagarwal/Visual_Studio_Projects/Excel/SUNNY.MSC4SEM.CHEMISTRY.2024.xls'  # Replace with your actual file path
output_file = '/Users/swapnilagarwal/Visual_Studio_Projects/Excel/RESULT_MSC4SEM_CHEMISTRY_RESULTS.xls'  # Replace with your desired output file path
...
columns = {
    ...
    'Practical_2_Name': 11,
    'Practical_3_Name': 12,
    'Subject_1_Main_Marks': 13,
    'Subject_1_Main_Marks_Suffix': 14,
    'Subject_1_CCE': 15,
    'Subject_1_CCE_Suffix': 16,
    'Subject_1_Total': 17,
    'Subject_1_Total_Suffix': 18,
    'Subject_2_Main_Marks': 19,
    'Subject_2_Main_Marks_Suffix': 20,
    'Subject_2_CCE': 21,
    'Subject_2_CCE_Suffix': 22,
    'Subject_2_Total': 23,
    'Subject_2_Total_Suffix': 24,
    'Subject_3_Main_Marks': 25,
    'Subject_3_Main_Marks_Suffix': 26,
    'Subject_3_CCE': 27,
    'Subject_3_CCE_Suffix': 28,
    'Subject_3_Total': 29,
    'Subject_3_Total_Suffix': 30,
    'Subject_4_Main_Marks': 31,
    'Subject_4_Main_Marks_Suffix': 32,
    'Subject_4_CCE': 33,
    'Subject_4_CCE_Suffix': 34,
    'Subject_4_Total': 35,
    'Subject_4_Total_Suffix': 36,
    'Practical': 37,
    'Practical_Suffix': 38,
    'Practical_1': 39,
    'Practical_1_Suffix': 40,
    'Practical_2': 41,
    'Practical_2_Suffix': 42,
    'Practical_3': 43,
    'Practical_3_Suffix': 44,
    'Theory_Total': 45,
    'Practical_Total': 46,
    'SEM_1': 47,
    'SEM_2': 48,
...
def split_subject(subject, char_limit=30):
...
for index, row in df.iterrows():
    subject_1_first, subject_1_second = split_subject(row.iloc[columns['Subject_1']])
    subject_2_first, subject_2_second = split_subject(row.iloc[columns['Subject_2']])
    subject_3_first, subject_3_second = split_subject(row.iloc[columns['Subject_3']])
    subject_4_first, subject_4_second = split_subject(row.iloc[columns['Subject_4']])
    transformed_row = [
        # Row 1
        [
            index + 1, 
        ...
        [
            ...
            '', 
            '', 
            row.iloc[columns['Subject_5']], 
            '', '', '', '', '100', '40',
            (str(row.iloc[columns['Practical']]) if not pd.isna(row.iloc[columns['Practical']]) else '') + 
            (str(row.iloc[columns['Practical_Suffix']]) if not pd.isna(row.iloc[columns['Practical_Suffix']]) else ''), 
            '',
            (str(row.iloc[columns['Practical']]) if not pd.isna(row.iloc[columns['Practical']]) else '') + 
            (str(row.iloc[columns['Practical_Suffix']]) if not pd.isna(row.iloc[columns['Practical_Suffix']]) else '')
        ],
        ...
        [
            ...
            '', 
            '    THEORY MARKS:' + str(row.iloc[columns['Theory_Total']]) + '/500',
            '    PRACTICAL MARKS:' + str(row.iloc[columns['Practical_Total']]) + '/200', 
            '  TOTAL MARKS:' + str(row.iloc[columns['Theory_Total']] + row.iloc[columns['Practical_Total']]) + '/700', 
            '', '', '', '', ''
        ],
        # Row 11
        [
            'I  SEM.:', 
            (str(row.iloc[columns['SEM_1']]) if not pd.isna(row.iloc[columns['SEM_1']]) else '') + '/600', 
            'II SEM.' + (str(row.iloc[columns['SEM_2']]) if not pd.isna(row.iloc[columns['SEM_2']]) else '') + '/600         III SEM.:',  
            (str(row.iloc[columns['SEM_3']]) if not pd.isna(row.iloc[columns['SEM_3']]) else '') + '/600           IV SEM.:' + 
            (str(row.iloc[columns['SEM_4']]) if not pd.isna(row.iloc[columns['SEM_4']]) else '') + '/700        GRAND',
            'TOT.: ', 
            (str(row.iloc[columns['GRAND_TOTAL']]) if not pd.isna(row.iloc[columns['GRAND_TOTAL']]) else '') + '/2500', 
            '', '',
