...
input_file = '/Users/swapnilagarwal/Visual_Studio_Projects/Excel/SUNNY.MSC4SEM.MATHS.2024.xls'  # Replace with your actual file path
output_file = '/Users/swapnilagarwal/Visual_Studio_Projects/Excel/RESULTS_MATHS_RESULTS.xls'  # Replace with your desired output file path
...
for index, row in df.iterrows():
    ...
    subject_5_first, subject_5_second = split_subject(row.iloc[columns['Subject_5']])
    transformed_row = [
        # Row 1
        [
            index + 1, 
        ...
        [
            'I  SEM.:', 
            (str(row.iloc[columns['SEM_1']]) if not pd.isna(row.iloc[columns['SEM_1']]) else '') + '/500', 
            'II SEM.' + (str(row.iloc[columns['SEM_2']]) if not pd.isna(row.iloc[columns['SEM_2']]) else '') + '/500         III SEM.:',  
            (str(row.iloc[columns['SEM_3']]) if not pd.isna(row.iloc[columns['SEM_3']]) else '') + '/500           IV SEM.:' + 
            (str(row.iloc[columns['SEM_4']]) if not pd.isna(row.iloc[columns['SEM_4']]) else '') + '/500        GRAND',
            'TOT.: ', 
            (str(row.iloc[columns['GRAND_TOTAL']]) if not pd.isna(row.iloc[columns['GRAND_TOTAL']]) else '') + '/2000', 
            '', 
